...
u8 *shm_key = "coordinator";   /*SHM_KEY for coordinator*/

static u32 branch_edge_pair[MAP_SIZE];       /* Map of edge pairs */
static u32 branch_edge_score[MAP_SIZE];      /* Map of edge score */
...
struct queue_entry
{
  ...
  int edge_score; /* Edge score of Seed       */

  u32 seed_id;    /* Id of the seed       */
...
struct edge_info
{
  u32 seed_id;   /* Id of seed which can reach the edge*/
  u32 hit_count; /* Total number of executions of the edge */
};
...
static int cur_edge_score = -1; /* Edge score of executed input       */
static int max_edge_score = -1; /* Maximal Edge score for any input   */
static int min_edge_score = -1; /* Minimal Edge score for any input   */
...
static void load_branch_map()
{
  char *fn = alloc_printf("branchMap.out");
  ...
  int match;
  char *format = malloc(strlen(out_dir) + 50);
  sprintf(format, "%%u\t%%u\t%%u\n");
  while (1)
  {
    u32 bb, fedge, sedge;
    match = fscanf(f, format, &bb, &fedge, &sedge);
    if (match == EOF)
    {
      break;
    }
    ...
    {
      PFATAL("Error parsing branchMap.out file; match = %d", match);
    }
    if (fedge >= MAP_SIZE || sedge >= MAP_SIZE)
    {
      PFATAL("Edge ID too large");
    }
    branch_edge_pair[fedge] = sedge;
    branch_edge_pair[sedge] = fedge;
  }
  free(format);
  fclose(f);
}

static void load_edge_score()
{
  char *fn = alloc_printf("edgeScore.out");
  int fd = open(fn, O_RDONLY);
  if (fd < 0)
    PFATAL("Unable to open '%s'", fn);
  ck_free(fn);
  ...
  {
    u32 edge, edgeScore;
    match = fscanf(f, format, &edge, &edgeScore);
    if (match == EOF)
    {
      break;
    }
    ...
    {
      PFATAL("Error parsing edgeScore.out file; match = %d", match);
    }
    branch_edge_score[edge] = edgeScore;
  }
  free(format);
  fclose(f);
}

static int update_branch_edge_score(){
	int edge_num = 0;
  int total_score = 0;
  int average_score = 0;
	for(int i = 0; i < MAP_SIZE; i++){
		if(trace_bits[i] && branch_edge_pair[i]){
      edge_statistics[i].hit_count++;
      edge_statistics[i].seed_id = queued_paths;
      if(!trace_bits[branch_edge_pair[i]]){
        u32 edgeId = branch_edge_pair[i];
			  u32 edgeScore = branch_edge_score[edgeId];
			  if (edgeScore){
	...
	if (total_score > 0 && edge_num > 0){
		average_score = total_score / edge_num;
	}
	return average_score;
}

static void update_seed_id(u32 input_id){
  for (int i = 0; i < MAP_SIZE; i++)
  {
    if (trace_bits[i] && branch_edge_pair[i])
    {
      edge_statistics[i].seed_id = input_id;
    }
  }
}
...
static void add_to_queue(u8 *fname, u32 len, u8 passed_det)
{
  ...
  q->edge_score = cur_edge_score;
  q->seed_id = queued_paths;
  ...
  if (cur_edge_score > 0)
  {
    ...
    if (max_edge_score <= 0)
    {
      max_edge_score = cur_edge_score;
      min_edge_score = cur_edge_score;
    }
    if (cur_edge_score > max_edge_score)
      max_edge_score = cur_edge_score;
    if (cur_edge_score < min_edge_score)
      min_edge_score = cur_edge_score;
  }
...
EXP_ST void setup_shm(void)
{
  ...
  if (fd <= -1)
    PFATAL("shmopen() failed for coordinator");

  if (ftruncate(fd, MAP_SIZE * sizeof(edge_info_t)) != 0)
    PFATAL("ftruncate() error");
  edge_statistics = mmap(0, MAP_SIZE * sizeof(edge_info_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
  close(fd);
                         ...
                         u32 handicap, u8 from_queue)
{
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++)
  {
    ...
    if (q->edge_score <= 0)
    {
      ...
      cur_edge_score = update_branch_edge_score();
...
static void show_stats(void)
{
  ...
  banner_pad = (100 - banner_len) / 2;
  memset(tmp, ' ', banner_pad);

  sprintf(tmp + banner_pad, "%s " cLCY cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "AFL-SILK", use_banner);
...
static u32 calculate_score(struct queue_entry *q)
{
  ...
  if (q->edge_score > 0)
  {
      ...
      if(max_edge_score != min_edge_score){
        normalized_d = (double)(max_edge_score - q->edge_score) / (max_edge_score - min_edge_score);
      }
      if (normalized_d >= 0) {
